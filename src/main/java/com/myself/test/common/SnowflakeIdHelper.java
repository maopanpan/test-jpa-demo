...
public class SnowflakeIdHelper {
    ...
    private static Logger log = LoggerFactory.getLogger(SnowflakeIdHelper.class);
     ...
     */
    private static volatile long sequence = ThreadLocalRandom.current().nextInt(2);
    /**
     * 上次生成ID的时间截
     * 此字段涉及多线程并发写场景 设置volatile保障happens-before 让写立刻对其他线程可见
     */
     ...
     */
    public SnowflakeIdHelper(long dataCenterId) {
        if (dataCenterId == 0) {
            try {
